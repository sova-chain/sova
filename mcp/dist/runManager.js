...
export class RunAlreadyActiveError extends Error {
    ...
    constructor(activeRunId) {
        super(`a mine run is already active (runId ${activeRunId}). Call sova_stop first, or sova_status to check on it.`);
...
export async function startMineRun(opts) {
 ...
 */
export async function stopRun(runId, graceMs = 5_000) {
    const run = getInternalRun(runId);
    if (run.status !== "running") {
        return toPublic(run);
    }
