...
export class MinerBinaryMissingError extends Error {
    constructor() {
        super(`sova-miner binary not found at ${MINER_BIN}. Build it first:\n` +
...
export async function assertMinerBinaryExists() {
    try {
 ...
 */
export async function runMinerOnce(args, timeoutMs = 60_000) {
    await assertMinerBinaryExists();
    return new Promise((resolve, reject) => {
        execFile(MINER_BIN, args, { timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024 }, (error, stdout, stderr) => {
            if (error && typeof error.code !== "number" && error.signal) {
                // Killed by timeout/signal rather than a normal non-zero exit.
                reject(new Error(`sova-miner ${args.join(" ")} was killed (${error.signal}) after ${timeoutMs}ms:\n${stderr}`));
                return;
            }
...
//# sourceMappingURL=minerCli.js.map