...
import {
  DEFAULT_MINER_DATA_DIR,
  DEFAULT_RPC_URL,
  DEFAULT_SOVA_RPC_URL,
  MINER_BIN,
  REGTEST_HARNESS_DIR,
} from "./paths.js";
...
server.registerTool(
  "sova_init",
  {
    ...
    description:
      "Runs `sova-miner init`: creates (or loads) this miner's keystore and " +
    ...
    inputSchema: {
      dataDir: z
        ...
        .optional()
        .describe(
          `Directory for keystore.json/state.json (default: ${DEFAULT_MINER_DATA_DIR}).`,
        ),
      network: NetworkEnum.describe(
        "Zcash network to build addresses/transactions for. Must match the node behind --rpc later.",
      ),
      evmAddress: z
        .string()
        .optional()
        .describe(
          "Credit this EVM address (20-byte hex, with or without 0x) instead of the keystore key's own Ethereum address. Only for an address whose key the user holds.",
        ),
    },
  },
  async ({ dataDir, network, evmAddress }) => {
    ...
    if (evmAddress) args.push("--evm-address", evmAddress);

    try {
      const result = await runMinerOnce(args);
      if (result.exitCode !== 0) {
        return fail(
          `sova-miner init exited ${result.exitCode}\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
        );
      }
...
server.registerTool(
  "sova_fund_regtest",
  {
    ...
    description:
      "Regtest-only convenience: calls the zebrad `generatetoaddress` RPC " +
    ...
    inputSchema: {
      dataDir: z
        ...
        .optional()
        .describe(
          `Miner data dir to read the funding address from, if --address is omitted (default: ${DEFAULT_MINER_DATA_DIR}).`,
        ),
      rpcUrl: z
        .string()
      ...
      blocks: z
        ...
        .default(101)
        .describe(
          "Number of blocks to mine to the address (101 = 1 mature coinbase).",
        ),
      address: z
        .string()
        .optional()
        .describe(
          "Override: mine to this t-addr instead of reading it from the miner's state.",
        ),
    },
  },
  async ({ dataDir, rpcUrl, blocks, address }) => {
    try {
      ...
      if (!target) {
        ...
        if (!state) {
          return fail(
            `no address given and no miner state found at ${resolvedDataDir}/state.json -- run sova_init first, or pass an explicit address.`,
          );
        }
...
server.registerTool(
  "sova_mine",
  {
    ...
    description:
      "Starts `sova-miner mine` as a background child process: while budget " +
    ...
    inputSchema: {
      budgetZat: z
        ...
        .positive()
        .describe(
          "Total zatoshis (burn + fee, across the whole run) this run may spend.",
        ),
      perEpochZat: z
        .number()
        .int()
      ...
      sovaRpcUrl: z
        ...
        .optional()
        .describe(
          `SIP-8: JSON-RPC URL of the user's OWN Sova node, passed as --sova-rpc (default: ${
            DEFAULT_SOVA_RPC_URL ?? "none -- SOVA_NODE_RPC_URL is not set"
          }). Pass "" to mine without votes. Never point it at someone else's node: that hands them the vote.`,
        ),
      voteWaitSecs: z
        .number()
        .int()
        ...
        .optional()
        .describe(
          "SIP-8: --vote-wait, seconds to wait for the Sova block anchoring a new Zcash block before voting for the head as it is (CLI default 10). Ignored without a Sova node.",
        ),
    },
  },
  async ({
...
server.registerTool(
  "sova_status",
  {
    ...
    description:
      "Tails a mine run's log and summarizes progress (epochs completed, " +
...
server.registerTool(
  "sova_stop",
  {
    ...
    description:
      "Stops a running mine loop (SIGTERM, escalating to SIGKILL after a " +
...
server.registerTool(
  "sova_report",
  {
    ...
    description:
      "Runs `sova-miner report`, optionally with --verify-rpc to independently " +
    ...
    inputSchema: {
      ...
      verifyRpcUrl: z
        ...
        .optional()
        .describe(
          "If given, cross-check the report against this node's full chain history.",
        ),
    },
  },
  async ({ dataDir, network, verifyRpcUrl }) => {
    ...
    if (verifyRpcUrl) args.push("--verify-rpc", verifyRpcUrl);
...
async function main() {
  ...
  console.error(
    `sova-miner-mcp: connected over stdio (binary: ${MINER_BIN}, active run: ${
      findActiveRun()?.runId ?? "none"
    }, known runs: ${listRuns().length})`,
  );
}
