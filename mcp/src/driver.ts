...
function log(section: string, payload: unknown) {
  console.log(`\n### ${section}\n`);
...
async function main() {
  ...
  const dataDir =
    process.env.SOVA_DRIVER_DATA_DIR ??
  ...
  if (init.isError) throw new Error("sova_init failed");
  const initJson = init.json as { fundingAddress?: string } | undefined;
  const fundingAddress = initJson?.fundingAddress;
  if (!fundingAddress) throw new Error("sova_init did not return a fundingAddress");
  ...
  if (fund.isError) throw new Error("sova_fund_regtest failed");
  ...
  const autoMine = spawn(
    path.join(HERE, "..", "..", "box", "regtest", "auto-mine.sh"),
    ["2", rpcUrl],
    { stdio: "ignore" },
  );
  ...
  try {
    ...
    if (mine.isError) throw new Error("sova_mine failed");
    const runId = (mine.json as { runId?: string })?.runId;
    if (!runId) throw new Error("sova_mine did not return a runId");
    ...
    if (!rejected.isError) {
      throw new Error(
        "expected sova_mine to refuse a second concurrent run, but it did not error",
      );
    }
    ...
    for (let attempt = 0; attempt < 60; attempt += 1) {
      ...
      statusJson = status.json as Record<string, unknown>;
    ...
    if ((statusJson.epochsCompleted as number) < 5) {
      throw new Error(
        `expected 5 epochs completed, got ${statusJson.epochsCompleted}`,
      );
    }

    // 6. stop (no-op if already exited -- proves the tool handles that path)
    const stop = await callTool(client, "sova_stop", { runId });
    if (stop.isError) throw new Error("sova_stop failed");
    ...
    if (report.isError) throw new Error("sova_report failed");
    const reportJson = report.json as { matches?: boolean } | undefined;
    ...
    if (mine2.isError) throw new Error("sova_mine (phase 2) failed");
    const runId2 = (mine2.json as { runId?: string })?.runId;
    if (!runId2) throw new Error("sova_mine (phase 2) did not return a runId");
    ...
    for (let attempt = 0; attempt < 15; attempt += 1) {
      ...
      const j = status.json as { status?: string; epochsCompleted?: number };
    ...
    if (!sawRunning) throw new Error("phase 2 run was never observed in 'running' status");
    if (!sawEpoch) throw new Error("phase 2 run never completed an epoch before timeout");

    const stop2 = await callTool(client, "sova_stop", { runId: runId2, graceMs: 5000 });
    if (stop2.isError) throw new Error("sova_stop (phase 2) failed");
    const stop2Json = stop2.json as { status?: string };
    if (stop2Json.status !== "stopped") {
      throw new Error(
        `expected phase 2 run to be in status "stopped" after sova_stop, got ${stop2Json.status}`,
      );
    }

    const finalStatus = await callTool(client, "sova_status", { runId: runId2 });
    const finalJson = finalStatus.json as { status?: string; exitSignal?: string | null };
    ...
    if (stopAgain.isError) throw new Error("sova_stop (idempotent re-stop) failed");
