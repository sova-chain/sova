...
- Status honesty: “ship with the public testnet, which will be the first public network”; SIP-6 “is a draft” (Accepted 2026-09-23, not yet built; the paper now cites it as accepted); wrapped ZEC is one sentence of future work.

## 7. Corrections after the reorg audit (2026-09-23)

`docs/audits/2026-09-23-reorg-and-fork-choice.md` found that the v5 text over-stated finality. What changed in the paper (commit "paper: correct the rewrite and finality claims after the reorg audit"), superseding the rows above where they differ:

- **Finality splits in two.** Mints are final at Zcash depth (rewriting one needs a Zcash reorg from E_N). Transaction history is not: Zcash alone does not choose between two Sova histories that settle the same epochs, and Sova adds no proof-of-work. §1 row 3.3, §3 "Fork choice follows Zcash" and "Rewriting a mint requires a Zcash reorg" hold for mints only.
- **The branch rule** (replaces the sibling rule, 2026-09-23; audit F1 follow-up). The sibling rule (a block is a candidate for a height only if it extends the node's own block at the height before) made any split permanent, as the nightly ladder scenario showed. A block is now a candidate if its ancestry, through blocks the node holds, meets the node's chain and, where it leaves it, extends the head or is preferred to the node's block there, replacing at most three of the node's blocks (`MAX_REPLACE_DEPTH = 3`, `crates/engine/src/candidates.rs` on `engine/sibling-rule`, shipped together with this text); candidates are compared by their blocks where their branches part, so splits up to three blocks deep heal. §5, §6 step 5, §10 and §11 say so: a node never replaces a block once three are built on it, so a transaction is settled once three more epochs are built on its block (about four minutes, 3 × 75 s), not one; the late rank-0 win is still the common one-block replacement at the tip. §10 adds that within those three blocks a rewrite is cheap until SIP-6 ships (audit F5), and that a split deeper than three blocks does not heal by itself. §3 and §11 say "near the tip" where they said "at the tip".
