...
| **Keeper miner** (Zcash t-addr + its EVM address) | `setup-host.sh` on `sova-keeper-1`, the AWS EC2 instance (Rob, 2026-09-23; `docs/ops/keeper-aws.md`), via `sova-miner init` | `/var/lib/sova/keeper/` on that instance's encrypted EBS root disk only, mode 600. Never copied off it (no EBS snapshots of it, no copy on the laptop) | None. It is a disclosed testnet miner, so a new key is fine. Rebuilding the instance makes a new one. | The thief gets its TAZ (topped up one run's budget at a time, about 0.35 TAZ a day) and its mined testnet SOVA. They could also burn as "the keeper", which only earns them testnet SOVA. | `init` a new keystore, publish the new addresses in the disclosure, fund the new t-addr, restart `sova-keeper` and `sova-node`. |
